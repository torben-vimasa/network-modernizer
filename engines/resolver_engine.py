...
class ResolverEngine:
    ...
    def resolve_source(self, source_ip):
        ...
        for node in self.graph.nodes.values():
            ...
            if node.type not in ["RouterInterface", "ASAInterface", "Interface"]:
                continue
                ...
                candidates.append(
                    {
                        ...
                        "vrf": context,
                        "reason": (
                            f"Source {source_ip} is in subnet {network} "
                            f"on ASA interface {node.name}"
                        ),
                        "confidence": "medium"
                    }
                )
        ...
        if len(candidates) > 1:
            return {
                ...
                "reason": f"Source {source_ip} matched {len(candidates)} interfaces",
