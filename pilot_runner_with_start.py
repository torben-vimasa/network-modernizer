...
def load_pilot_files(path: Path) -> list[dict[str, Any]]:
    ...
    for file in files:
        ...
        if isinstance(content, list):
            file_pilots = content
        elif isinstance(content, dict) and isinstance(content.get("flows"), list):
...
def normalize_pilot(
    ...
    source = pilot.get("source") or pilot.get("src")
    destination = pilot.get("destination") or pilot.get("dst")
    ...
    name = pilot.get("name") or f"{source} to {destination}"
    ...
    return {
        ...
        "vrf": pilot.get("vrf"),
        "start": pilot.get("start"),
        "route_destination": pilot.get("route_destination"),
...
def run_trace(
    ...
    if pilot.get("start"):
        kwargs["start"] = pilot["start"]

    if pilot.get("route_destination"):
        kwargs["route_destination"] = pilot["route_destination"]
...
def explanation_lines(result) -> list[str]:
    ...
    for attribute in [
        ...
        value = getattr(explanation, attribute, None)

        if isinstance(value, list):
            return [str(item) for item in value]

    try:
        if not isinstance(explanation, (str, bytes)):
...
def determine_status(result) -> str:
    ...
    if (
        ...
        and getattr(security, "permitted", None) is False
    ):
        return "denied"

    network_hops = getattr(result, "network_hops", None) or []

    for hop in network_hops:
        if getattr(hop, "policy", None) == "deny":
            return "denied"
...
def collect_devices(result) -> list[str]:
    ...
    for hop in getattr(result, "network_hops", None) or []:
        device = getattr(hop, "device", None)
...
def collect_acl_rules(result) -> list[str]:
    ...
    if security is not None:
        ...
        match = getattr(security, "match", None)

        if match is not None:
            acl = getattr(match, "acl", None)
            sequence = getattr(match, "rule", None)
    ...
    for hop in getattr(result, "network_hops", None) or []:
        rule = getattr(hop, "acl_rule", None)
...
def evaluate_expectations(
    ...
    regression_passed = all(check["passed"] for check in checks)
...
def print_pilot_result(
    ...
    if evaluation["acl_rules"]:
        print("    ACL    : " + ", ".join(evaluation["acl_rules"]))
    ...
    if verbose:
        print(f"    File   : {pilot.get('_pilot_file')}")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Run NDT pilot flows as regression tests."
    )
    ...
    parser.add_argument(
        ...
        help="Pilot JSON file or directory. Default: pilots"
    )
    ...
    parser.add_argument(
        "--fail-on-observe",
        action="store_true",
        help="Treat pilots without expected results as failures."
    )
    ...
    try:
        raw_pilots = load_pilot_files(pilot_path)
        pilots = [normalize_pilot(pilot) for pilot in raw_pilots]
    ...
    if not pilots:
        print(
            f"No pilot definitions found in {pilot_path}",
            file=sys.stderr
        )
        return 2
    ...
    for number, pilot in enumerate(pilots, start=1):
        try:
